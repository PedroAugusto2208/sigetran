...
const { Usuario, Veiculo, Motorista, Cliente, Rota, Carga, Viagem, Frete } = require('./models');
const FreteService = require('./services/FreteService');

/**
 * Popula o banco com dados de demonstração.
 * NÃO recria o schema nem fecha a conexão — seguro para chamar no boot.
 * Idempotente: só roda se ainda não houver usuários cadastrados.
 */
async function popular() {
  const jaTem = await Usuario.count();
  if (jaTem > 0) return false;

  // ===================== USUÁRIOS =====================
  await Promise.all([
    Usuario.create({ login: 'admin',      senha: await bcrypt.hash('admin123', 10),  nome: 'Carlos Administrador', tipo: 'ADMINISTRADOR' }),
  ...
  // ===================== VIAGENS + FRETES =====================
  async function criarViagem(vi, mi, ri, ci, cli, status, diasAtras) {
    const veiculo = veiculos[vi], motorista = motoristas[mi], rota = rotas[ri], carga = cargas[ci], cliente = clientes[cli];
    const dataBase = new Date();
    dataBase.setDate(dataBase.getDate() - diasAtras);

    const viagem = await Viagem.create({
      VeiculoId: veiculo.id, MotoristaId: motorista.id, RotaId: rota.id,
      CargaId: carga.id, ClienteId: cliente.id, status,
    ...
    const calculado = FreteService.calcular({
      distanciaKm: Number(rota.distanciaKm), valorPedagio: Number(rota.valorPedagio),
      pesoKg: Number(carga.pesoKg), capacidadeCarga: Number(veiculo.capacidadeCarga),
      tipoMercadoria: carga.tipoMercadoria,
    });
    await Frete.create({ ...calculado, ViagemId: viagem.id });
  }

  await criarViagem(0, 0, 0, 0, 0, 'CONCLUIDA',    15);
  await criarViagem(1, 1, 3, 1, 2, 'CONCLUIDA',    12);
  await criarViagem(2, 2, 1, 2, 1, 'CONCLUIDA',     8);
  await criarViagem(0, 3, 5, 5, 4, 'CONCLUIDA',     5);
  await criarViagem(1, 0, 4, 6, 0, 'CONCLUIDA',     3);
  await criarViagem(3, 1, 2, 3, 3, 'EM_ANDAMENTO',  1);
  await criarViagem(2, 2, 0, 4, 2, 'EM_ANDAMENTO',  0);
  await criarViagem(0, 3, 1, 0, 1, 'PROGRAMADA',   -2);
  await criarViagem(1, 1, 3, 1, 2, 'PROGRAMADA',   -3);
  await criarViagem(3, 2, 5, 5, 4, 'PROGRAMADA',   -5);
